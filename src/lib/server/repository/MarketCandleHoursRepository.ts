...
import type { MarketCandleHoursEntity } from '$lib/server/entities/MarketCandleHoursEntity';
import { CurrentDateUtils } from '$lib/common/utils/CurrentDateUtils';
import { LoggingUtils } from '$lib/common/utils/LoggingUtils';

export const MarketCandleHoursRepository = {
	findAllByMarketAndBeginDateTimeAndCount: findAllByMarketAndBeginDateTimeAndCount,
	findAllByMarketCurrencyAndBeginDateTimeAndCountAndRowNumberLte: findAllByMarketCurrencyAndBeginDateTimeAndCountAndRowNumberLte,
...
async function findAllByMarketAndBeginDateTimeAndCount(
	...
	count: number,
): Promise<MarketCandleHoursEntity[]> {
	...
	try {
		...
		return result.rows as MarketCandleHoursEntity[];
	} catch (error) {
		console.error(
			`${CurrentDateUtils.getNowDateTimeString()} - ### MarketCandleHoursRepository.findAllByMarketAndBeginDateTimeAndCount Error: ${error}`
		);
		return [];
	} finally {
		dbConn.release();
...
async function findAllByMarketCurrencyAndBeginDateTimeAndCountAndRowNumberLte(
	...
	try {
		...
		LoggingUtils.error(
			'MarketCandleHoursRepository.findAllByMarketCurrencyAndBeginDateTimeAndCountAndRowNumberLte',
			`${error}`,
		)
		return [];
	} finally {
...
async function findAllByMarketLikeAndRowNumberLte(
	marketCurrency: string,
	rowNumber: number,
) {
	const dbConn = await connectToDB();
	...
	try {
		...
		LoggingUtils.error(
			'MarketCandleHoursRepository.findAllByMarketLikeAndRowNumberLte',
			`${error}`,
		)
		return [];
	} finally {
